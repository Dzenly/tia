...
`npm install [-g] tia`.
For test creation/debugging the local installation is more convenient.
...
which are auto-generated at typescript files compilation into tia/gen-types.

Note: When you use search in this documentation, mouse click on found result may not work,
...
JavaScript file, which name ends by `.tia.js` or `.tia.ts` suffix (e.g. `00_my-test.tia.js`, `01_my-test.tia.ts`).
...
TIA contains `d.ts` definitions for its API in gen-types subdir.
...
but the log has the `.tia.log` extension (test has the `.tia.js` or `.tia.ts` extenstion).
...
If the test named `xxxx.tia.js`/`xxxx.tia.ts` then dif will be named as `xxxx.tia.dif`
...
Browser profiles are located in `**/_tia-suite/tia-browser-profiles/<browserProfileDir>`  directories,
...
`*.tia.js` / `*.tia.ts` files are executed by TIA and should use TIA API to create [Test logs](#test-log).
...
https://github.com/Dzenly/tia/tree/master/tia-types
