...
* Идешь на какой-то URL. (см. s.get(url) ф-ю).
...
* Считываешь что-то из JS переменных браузера. (см. s.executeScript()).

### Замечания об ExtJs

* Для динамически генерируемых id нужно через разные обертки движка (пока их мало),
 или через s.executeScript находить id элемента и посылать в него разные действия.
