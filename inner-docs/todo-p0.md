...
* Проверить, что rootDir везде используется по назначению. Например, сломан сьют конфиг.

* Сделать рутовый лог.

* Доренамить meta и потом всякие slog - в log, et.
...
Чтобы в паттернах не париться с указанием суффикса.
Суффикс должен существовать для всех тестов.
