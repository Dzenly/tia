Клики элементов комбобокса по тексту и по id.
2 ф-и.
...
Пока не надо по лейблу искать.

API для помещения значения в textBox.

API для кликов по checkbox.
...
Вычитать всю форму, в смысле все значения.

Вычитать всю форму, в смысле все филды, распечатать их имя, лейбл, значение,
набор значений в store. 


На выходных подумать как перевести на TypeScript, можно ли переводить частично.
Как ставятся TypeScript npm модули.

Прочитать доки про другие элементы управления.

Selenium: id vs WebElement.
Есть ли возможность сликать по Id, или мы пройдем через WebElement.

Написать тест для всего API.
...
Считать значение в элементе формы по id.
Записать значение в элемент формы по id.

Вычитать все возможные значения из контролов, предлагающих выбор.

Найти элемент по name.

Найти элемент формы по метке.

Найти элемент по ключу локали для метки.

Вычитать полное состояние формы.
Может быть полезно для дефолтных состояний.

В исследовалке - выводить name и hiddenName элементов форм.
...
?? Как выбирать элементы из комбобоксов.
...
Вместо юзерского имени, может можно у таблиц выдернуть какой-то id, reference, itemId, заголовок, и т.д.
И у деревьев тоже.

Потестить апи для кликов для таблиц, в которых модель распространяется не на все поля.

expandAll в treeView и в features.Grouping для table.

Индекс строки, индекс столбца.
М.б. у столбца есть id?
...
Почитать про доступ к записи или к html элементу по id шнику.
Посмотреть, фигурирует ли id шник в CSS стилях элемента или в аттрибутах.

Распечатать полностью запись для элемента под курсором.
Если не получится, то распечатывать id шники содержимого в отдельный диалог ?
