Переделать всю распечатку инфы о компоненте на массивы строк.
Модуляризировать распечатку.
Где можно - задействовать tia.cU.dumpObj.

obj, propPaths, dstArr, safe

Проверить, что работает инфа о компоненте по клику.
Включая инфу о контроллерах.

Сделать ф-ю по распечатке всей видимой иерархии документа.
Однако, показывать grayed and disabled.
...
1. Вычитать getValues(), оттуда взять names, через name находить компонент,
у компонента находить возможного родителя.
