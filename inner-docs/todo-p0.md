...
Задавать параметр, отдельно помечать в subj слишком длинное выполнение теста.
Это прямо в префикс, после ET_MLOG.
Типа, тест длиннее, чем какой-то параметр.
Или, может набор параметров.

--too-long-time <duration> 

T0, - 0 - first ms.
T3 - after third ms.

Если нет параметра - то нет и метки в subj.
