/tmp - много профайлов браузеров ?

В трейсинг - добавить \n в начало, если его не было в конце.

Дополнительный файл, где будут логи и трейсинг.
Чтобы отделить вывод tia от вывода defensys сервера.

===================

Перевернуть дифы, которые идут в mlog, так он сразу будет начинаться с проблемы.
Или вообще все дифы перевернуть.
...
Архив должен начинаться от tests_dir.

===================

Версия движка в заголовок лога.
...
Остановка зависшего теста не работает когда тестов много.

Добавил обработку, но тоже может не сработать.
Тогда придется как-то убивать процесс chromedriver.

===================

Иногда при клике по комбобоксу он открывается и сразу закрывается.

===================

Close приводит к зависанию. Возможно, браузер теряет фокус. А close закрывает браузер, который
сейчас в "фокусе" webdriver. Пока убрал его и юзаю quit напрямую.

===================

Понять почему ExtJs не реагирует на слишком быстрые клики.
Избавиться от задержки перед кликами.

===================

? Считать ли pass для API вызовов?

Или может завести счетчик успешных действий, а pass оставить только для assertions.
