...
Слить e-br-search.js и e-br-dyn-id.js.
...
Ф-я для getId.
Возвращает id и флаг autogenerated.

Завести какой-то словарь объектов, с которыми работаешь.
Сопоставить некий свой innerId, description, id, autogenerated.


===================

Как бы сделать, чтобы методы поиска компонента были отдельно.
И ф-я для распечатки контента - отдельно.

И их можно было комбинировать.
...
Можно сделать чайном.

Ф-я для поиска выдает id, а дальше с ним работаешь.

Можно сделать by, и в ф-ях разруливать условием.


log

Давай будут ф-и. Где первым параметром будет способ нахождения объекта.
...
id контейнера и reference

id и down(query)


В итоге ф-я выдает getId(). Он может быть либо автогенеренный, либо нет.
Если нет - писать.
