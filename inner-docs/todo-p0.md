...
* Поренамить any в cmp.
* Не вносить путаницу через ренаминг tableview->table, так и называть. tableview, treeview.
* Попробовать унаследоваться от any? Допустим, чтобы во все компоненты попало
selectAll через Ctrl + a.
* compName пробить во все actions/checks/
* Посмотреть при наследовании интерфейсов что будет делать typedoc.
...
* Доделать распечатку таблиц. Экшны и распечатка деревьев. Распечатка форм.
...
* Придумать как выделить сразу несколько строк для table.
Возможно задаешь массив столбцов. А затем массив строк. Где внутри массивы по кол-ву столбцов.
Со значениями столбцов.
...
* ? EJ Explorer для таблиц и деревьев должен искать родительскую панель,
а не выдавать вложенный генерённый View.
