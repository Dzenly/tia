...
id теста?
тэг автоматизирован ли тест.


Первый запуск дает DIF_FROM_PREV.

// Исследовалка не работает на элементы деревьев.
И наверное на гриды не сработает.

Добавить поддержку тэгов в тестах.
Один из тэгов - приоритет.
...
Подправить документацию по конфигам для мейла.
Теперь они настраиваются через cmd line опцию, или через environment variable.
