...
* Поддержать headless chrome. Посмотреть разницу в скорости между headful and headless.

* Сделать cmd line опцию для поддержки нечестного, но быстрого и более стабильного API.

* Проверить как все работает на IE.

* Изучить поиск по xtype (up, down, ComponentQuery, getCmp, children).
...
* Удалять зареквайренные файлы.

* Инкрементально ли добавляется coverage.

* Имя сьютлога, всегда одинаковое, какое ?
* Имя рутлога всегда одинаковое ? Какое ?

* м.б. добавить к расширению файла tia ?
* .log, .et,
Suite лог будет называться suite.log, эталонный suite log - suite.et.
Рутовый root.log, root.et.
...
* rootLog ? - Теперь это обобщающий лог для всех suiteLogs.
