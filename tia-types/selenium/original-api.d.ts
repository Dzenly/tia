/* eslint-disable @typescript-eslint/no-explicit-any */

/**
 * gT.sOrig
 * It is too difficult to import TS definitions from selenium-webdriver, so here they are marked as 'any'.
 * Original JS Selenium WebDriver API.
 * https://seleniumhq.github.io/selenium/docs/api/javascript/module/selenium-webdriver/index.html
 */
export interface SeleniumOriginalApi {
  /**
   * https://seleniumhq.github.io/selenium/docs/api/javascript/module/selenium-webdriver/index_exports_By.html
   */
  by: any;

  /**
   * https://seleniumhq.github.io/selenium/docs/api/javascript/module/selenium-webdriver/chrome.html
   */
  chrome: any;

  /**
   * https://seleniumhq.github.io/selenium/docs/api/javascript/module/selenium-webdriver/http/index_exports_HttpClient.html
   */
  Client: any;

  /**
   * https://seleniumhq.github.io/selenium/docs/api/javascript/module/selenium-webdriver/http/index_exports_Executor.html
   */
  Executor: any;

  /**
   * https://seleniumhq.github.io/selenium/docs/api/javascript/module/selenium-webdriver/firefox.html
   */
  firefox: any;

  /**
   * https://seleniumhq.github.io/selenium/docs/api/javascript/module/selenium-webdriver/lib/input.html
   */
  input: any;

  /**
   * https://seleniumhq.github.io/selenium/docs/api/javascript/module/selenium-webdriver/index_exports_Key.html
   */
  key: any;

  /**
   * https://seleniumhq.github.io/selenium/docs/api/javascript/module/selenium-webdriver/lib/until.html
   */
  until: any;

  /**
   * https://seleniumhq.github.io/selenium/docs/api/javascript/module/selenium-webdriver/index.html
   */
  wdModule: any;
}
